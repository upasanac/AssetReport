...
@Component(
		immediate=true, 
		metatype=true,
		label="MIS Weekly Asset report Job", 
		description="Scheduled Job to email Asset Weekly Report"
)
@Service
@Properties({
    @Property(label = "Enable", name = "enabled",description = "Enable/Disable this JOB", boolValue = false),
    @Property(label = "Scheduler Expression", name = "scheduler.expression", description = "Default is SUNDAY 00 HOURS", value = "0 0 0 ? * SUN *"),
...
public class MISWeeklyReportJob implements Runnable, ClusterAware {
	...
	Boolean sendWeeklyReport = false;	
	String reportType = MISReportConstant.ASSET_ADDED;
	private final String reportFrequency = MISReportConstant.WEEKLY;
    ...
    public void init(final ComponentContext componentContext) {
        ...
        sendWeeklyReport = (Boolean)(dictionary.get("enabled"));
    }	
