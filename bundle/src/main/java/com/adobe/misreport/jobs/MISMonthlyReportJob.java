...
@Component(
		...
		description="Scheduled Job to email Asset Monthly Report"
)
@Service
@Properties({
    @Property(label = "Enable", name = "enabled", description = "Enable/Disable this JOB", boolValue = false),
    @Property(label = "Scheduler Expression", name = "scheduler.expression", description = "Default is 1st of every month at 00 HOURS", value = "0 0 0 1 1/1 ? *"),
...
public class MISMonthlyReportJob implements Runnable, ClusterAware {
	...
	private final String reportFrequency = MISReportConstant.MONTHLY;
    ...
    public void init(final ComponentContext componentContext) {
        ...
        sendMonthlyReport = (Boolean)(dictionary.get("enabled"));
    }	
